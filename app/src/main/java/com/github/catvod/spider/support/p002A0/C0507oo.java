package com.github.catvod.spider.support.p002A0;

/* JADX INFO: renamed from: com.github.catvod.spider.support.A0.oo */
/* JADX INFO: loaded from: classes.dex */
public static final class C0507oo extends EnumC0709wa {
    public C0507oo() {
        super("Comment", 46);
    }

    @Override // com.github.catvod.spider.support.p002A0.EnumC0709wa
    /* JADX INFO: renamed from: a */
    public final void mo870a(C0316hm c0316hm, C0606sf c0606sf) {
        char cM1662aa = c0606sf.m1662aa();
        if (cM1662aa == 0) {
            c0316hm.m1289aj(this);
            c0606sf.m1676q();
            c0316hm.f914p.m1261d((char) 65533);
        } else if (cM1662aa == '-') {
            c0316hm.m1292w(EnumC0709wa.f1675ax);
        } else {
            if (cM1662aa != 65535) {
                c0316hm.f914p.m1262e(c0606sf.m1684y('-', 0));
                return;
            }
            c0316hm.m1288ai(this);
            c0316hm.m1285af();
            c0316hm.m1291al(EnumC0709wa.f1698c);
        }
    }
}
